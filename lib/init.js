...
const { createTypeData, installTypeData } = require("../util/default");
const { log } = console;
...
async function init(args) {
  ...
  try {
    ...
    const packageResult = handlebars.compile(packageContent)(initPrompt);
    fs.writeFileSync(packagePath, packageResult);

    spinner.succeed(chalk.green("已完成下载！\n"));

    const { installType } = await inquirerPrompt(installTypeData);
    ...
    spinner.succeed(chalk.green("初始化成功！（success）\n"));
  } catch (err) {
    spinner.stop();
    log(chalk.red(err));
    process.exit();
